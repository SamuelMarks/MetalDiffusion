        ...
        with tf.device(self.device):
            ...
            if input_image is not None:
                ...
                if isinstance(input_image, np.ndarray):
                    ...
                    input_image = keras.ops.convert_to_tensor(input_image, dtype = keras.config.floatx())
                    ...
                    inputImageArray = keras.initializers.Constant(input_image, dtype = keras.config.floatx())
                    inputImageArray = keras.ops.expand_dims(input_image[..., :3], axis = 0)
                    input_image_tensor = keras.ops.cast((inputImageArray / 255.0) * 2 - 1, self.dtype)
            ...
            if type(input_mask) is str:
                ...
                latent_mask_tensor = keras.ops.cast(keras.ops.repeat(latent_mask, batch_size , axis = 0), self.dtype)
            else:
            ...
            if sampler == "DPMSolver":
                ...
                if self.controlNet is not None:
                    controlNetImage = [keras.initializers.Constant(controlNetImage[0].copy(), dtype = keras.config.floatx()) / 255.0]
    ...
    def encodeText(
        ...
        with tf.device(self.device):
            if legacy is True:
                ...
                # Phrase - aka the prompt
                phrase = keras.ops.concatenate([[49406], inputs, [49407] * (TextLimit - len(inputs))], axis=0)
