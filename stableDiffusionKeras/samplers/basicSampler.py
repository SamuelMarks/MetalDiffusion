...
class BasicSampler():
    ...
    def get_x_prev_and_pred_x0(
        ...
        dir_xt = keras.ops.sqrt(keras.initializers.Constant(1.0) - a_prev - keras.ops.square(sigma_t)) * e_t
    ...
    def sample(
        ...
        with tf.device(device):
            ...
            for index, timestep in list(enumerate(self.timesteps))[::-1]:
                ...
                t_emb = keras.ops.repeat(t_emb, self.batchSize, axis = 0) #shape is (1, 320)
                ...
                if controlNet[0] is True:
                    ...
                    if controlNet[2] is None:
                        ...
                        controlNetUnconditionalArray = self.model.controlNet(
                            [self.latent, t_emb, unconditionalContext, keras.ops.concatenate(self.controlNetInput, axis = 3)],
                            training = False
                        )
                        controlNetConditionalArray = self.model.controlNet(
                            [self.latent, t_emb, context, keras.ops.concatenate(self.controlNetInput, axis = 3)],
                            training = False
                        )
                ...
                if vPrediction is False:
                    ...
                    if latentPrevious.dtype != self.latent.dtype:
                        latentPrevious = keras.ops.cast(latentPrevious, dtype = self.latent.dtype)
    ...
    def timestepEmbedding(
        ...
        half = dimensions // 2
        freqs = keras.ops.exp(
            -keras.ops.log(max_period) * keras.ops.arange(0, half, dtype = keras.config.floatx()) / half
        )
        args = keras.ops.convert_to_tensor([timesteps], dtype = keras.config.floatx()) * freqs
